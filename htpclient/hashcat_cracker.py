...
class HashcatCracker:
    def __init__(self, cracker_id, binary_download):
        ...
        self.executable_name = self.executable_name[:k] + \
        ...
        if not os.path.isfile(os.path.join(self.cracker_path, self.callPath)):
            self.executable_name = binary_download.get_version()['executable']
            k = self.executable_name.rfind(".")
            self.executable_name = self.executable_name[:k] + \
                get_bit() + "." + self.executable_name[k + 1:]
            self.cracker_path = "crackers/" + str(cracker_id) + "/"
            self.callPath = self.executable_name
            if Initialize.get_os() != 1:
                self.callPath = "./" + self.callPath
        ...
        try:
            ...
            output = subprocess.check_output(
                cmd, shell=True, cwd=self.cracker_path)
    ...
    def build_command(self, task, chunk):
        ...
        if 'useBrain' in task and task['useBrain']:
            ...
            if 'brainFeatures' in task:
                args += " --brain-client-features " + \
                    str(task['brainFeatures'])
        else:  # remove should only be used if we run without brain
            args += " --potfile-disable --remove --remove-timer=" + \
                str(task['statustimer'])
        args += " " + update_files(task['attackcmd']).replace(task['hashlistAlias'],
                                                              "../../hashlists/" + str(task['hashlistId'])) + " " + task['cmdpars']
        if args.find(" -S") != -1:
    ...
    def build_pipe_command(self, task, chunk):
        ...
        pre_args = " --stdout -s " + \
            str(chunk['skip']) + " -l " + str(chunk['length']) + ' '
        pre_args += update_files(task['attackcmd']
    ...
    def build_prince_command(self, task, chunk):
        ...
        if Initialize.get_os() != 1:
            ...
            binary += "exe"
        pre_args = " -s " + str(chunk['skip']) + \
            " -l " + str(chunk['length']) + ' '
        pre_args += get_wordlist(update_files(
    ...
    def build_preprocessor_command(self, task, chunk, preprocessor):
        ...
        if preprocessor['skipCommand'] is not None and preprocessor['limitCommand'] is not None:
            pre_args = " " + preprocessor['skipCommand'] + " " + str(
                chunk['skip']) + " " + preprocessor['limitCommand'] + " " + str(chunk['length']) + ' '
        else:
            pre_args = ""
        ...
        if preprocessor['skipCommand'] is None or preprocessor['limitCommand'] is None:
            pre_args += " | head -n " + \
                str(chunk['skip'] + chunk['length']) + \
                " | tail -n " + str(chunk['length'])
    ...
    def run_chunk(self, task, chunk, preprocessor):
        ...
        if 'usePrince' in task and task['usePrince']:  # DEPRECATED
            ...
            full_cmd = self.build_preprocessor_command(
                task, chunk, preprocessor)
        elif self.usePipe:
        ...
        if os.path.exists("hashlists/" + str(task['hashlistId']) + ".out"):
            if self.config.get_value('outfile-history'):
                os.rename("hashlists/" + str(task['hashlistId']) + ".out", "hashlists/" + str(
                    task['hashlistId']) + "_" + str(time.time()) + ".out")
            else:
        ...
        if Initialize.get_os() != 1:
            process = subprocess.Popen(full_cmd, shell=True, stdout=subprocess.PIPE,
                                       stderr=subprocess.PIPE, cwd=self.cracker_path, preexec_fn=os.setsid)
        else:
            process = subprocess.Popen(
                full_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=self.cracker_path)

        logging.debug("started cracking")
        out_thread = Thread(target=self.stream_watcher,
                            name='stdout-watcher', args=('OUT', process.stdout))
        err_thread = Thread(target=self.stream_watcher,
                            name='stderr-watcher', args=('ERR', process.stderr))
        ...
        main_thread = Thread(target=self.run_loop,
                             name='run_loop', args=(process, chunk, task))
        main_thread.start()
                    ...
                    if status.is_valid():
                        ...
                        if enable_piping and not self.uses_slow_hash_flag and ('useBrain' not in task or not task['useBrain']) and 'slowHash' in task and task['slowHash'] and not self.usePipe:
                            if task['files'] and not ('usePrince' in task and task['usePrince']) and not ('usePreprocessor' in task and task['usePreprocessor']) and 1 < self.statusCount < 10 and status.get_util() != -1 and status.get_util() < piping_threshold:
                                ...
                                ) / (chunk['skip'] + chunk['length']) * chunk['skip'])
                        ...
                        if total > 0:
                            relative_progress = int(
                                (status.get_progress() - chunk_start) / float(total - chunk_start) * 10000)
                        ...
                        while self.cracks or initial:
                            ...
                            if ans is None:
                                ...
                                logging.error(
                                    "Error from server on solve: " + str(ans))
                                try:
                                    kill_hashcat(proc.pid, Initialize.get_os())
                                except ProcessLookupError:
                                    pass
                                ...
                                if zaps:
                                    ...
                                    zap_output = "\tFF\n".join(zaps) + '\tFF\n'
                                    f = open(
                                        "hashlist_" + str(task['hashlistId']) + "/" + str(time.time()), 'a')
                                    f.write(zap_output)
                                    f.close()
                                logging.info("Progress:" + str("{:6.2f}".format(relative_progress / 100)) + "% Speed: " + print_speed(speed) + " Cracks: " + str(
                                    cracks_count) + " Accepted: " + str(ans['cracked']) + " Skips: " + str(ans['skipped']) + " Zaps: " + str(len(zaps)))
                            self.lock.release()
                    else:
    ...
    def measure_keyspace(self, task, chunk):
        ...
        full_cmd = self.callPath + " --keyspace --quiet " + \
            update_files(task['attackcmd']).replace(
                task['hashlistAlias'] + " ", "") + ' ' + task['cmdpars']
        if 'useBrain' in task and task['useBrain']:
            full_cmd += " -S"
        if Initialize.get_os() == 1:
            full_cmd = full_cmd.replace("/", '\\')
        ...
        try:
            logging.debug("CALL: " + full_cmd)
            output = subprocess.check_output(
                full_cmd, shell=True, cwd=self.cracker_path, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            logging.error("Error during keyspace measure: " +
                          str(e) + " Output: " + output.decode(encoding='utf-8'))
    ...
    def prince_keyspace(self, task, chunk):
        ...
        full_cmd = binary + " --keyspace " + \
            get_wordlist(update_files(task['attackcmd'], True).replace(
                task['hashlistAlias'], ""))
        if Initialize.get_os() == 1:
            full_cmd = full_cmd.replace("/", '\\')
        try:
            logging.debug("CALL: " + full_cmd)
            output = subprocess.check_output(
                full_cmd, shell=True, cwd="prince")
    ...
    def preprocessor_keyspace(self, task, chunk):
        ...
        # in case there is no keyspace flag, we just assume the task will be that large to run forever
        if preprocessor['keyspaceCommand'] is None:
        ...
        full_cmd = binary + " " + preprocessor['keyspaceCommand'] + \
            " " + update_files(task.get_task()['preprocessorCommand'])
        if Initialize.get_os() == 1:
            full_cmd = full_cmd.replace("/", '\\')
        try:
            logging.debug("CALL: " + full_cmd)
            output = subprocess.check_output(
                full_cmd, shell=True, cwd="preprocessor/" + str(task.get_task()['preprocessor']))
        except subprocess.CalledProcessError:
            logging.error("Error during preprocessor keyspace measure")
            send_error("Preprocessor keyspace measure failed!", self.config.get_value(
                'token'), task.get_task()['taskId'], None)
            sleep(5)
            return False
        output = output.decode(
            encoding='utf-8').replace("\r\n", "\n").split("\n")
    ...
    def run_benchmark(self, task):
        ...
        proc = subprocess.Popen(full_cmd, shell=True, stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE, cwd=self.cracker_path)
    ...
    def run_speed_benchmark(self, task):
        ...
        try:
            ...
            logging.error("Error during speed benchmark, return code: " +
                          str(e.returncode) + " Output: " + output.decode(encoding='utf-8'))
            send_error("Speed benchmark failed!", self.config.get_value(
                'token'), task['taskId'], None)
            return 0
        output = output.decode(
            encoding='utf-8').replace("\r\n", "\n").split("\n")
        benchmark_sum = [0, 0]
    ...
    def run_health_check(self, attack, hashlist_alias):
        ...
        args += update_files(attack).replace(hashlist_alias,
                                             "../../hashlists/health_check.txt")
